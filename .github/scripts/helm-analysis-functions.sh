...
EOF
    ...
    # Return the HTTP status code directly
    return "$http_status"
}
apply_ai_label() {
    local ai_analysis="$1"
...
perform_ai_analysis() {
    ...
    local http_status
    http_status=$(call_ai_api)
    
    if [[ $http_status -eq 200 ]]; then
        process_ai_response
    else
        local error_details
        error_details=$(get_error_message_for_status "$http_status")
        
        # Add error message from response if available
        if [[ -s ai_response.json ]]; then
            local error_msg=$(jq -r '.error.message // .message // empty' ai_response.json 2>/dev/null)
            if [[ -n "$error_msg" ]]; then
                error_details="$error_details - $error_msg"
            fi
        fi
        
        log_error "AI API call failed: $error_details"
        apply_ai_fallback_label "AI analysis unavailable"
    fi
}
